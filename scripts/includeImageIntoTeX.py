...
def getImgNames(oneValueName):
    imgFiles='/home/peilong/Documents/talks_latex/image/Hbb/VHbbModelingComparisonNewNOldRadhigh/*' + oneValueName + '*'
    imgNames=[os.path.basename(x) for x in sorted(glob(imgFiles))]
...
# open the section1.tex file
    print "opening section1.tex"
    print "remember to have the images in position before running the script"
    print "remember to set the turn-on marker (%% use_includeImageIntoTeX_py=true)"

    with open("./src/section1.tex", "rt") as fin:
        with open("./src/section1Temp.tex", "wt") as fout:
            # for loop
            print "reading section1.tex"
            for line in fin:
                # where to insert the line
                if 'use_includeImageIntoTeX_py=true' in line:
                    print "turn-on marker is set -> %% use_includeImageIntoTeX_py=true"
                    # get one valueName
                    fout.write(line)
